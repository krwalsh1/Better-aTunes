...
public class ArtistTest {
	...
	private Artist artist = new Artist(ARTIST_NAME);
	...
	public void testisEmpty() {
		Assert.assertTrue(artist.isEmpty());
		...
		a1.addAudioFile(af1);
		artist.addAlbum(a1);
		Assert.assertFalse(artist.isEmpty());
	}
	
	@Test
	public void testToString() {
		Assert.assertTrue(artist.toString().equals(ARTIST_NAME) );
	}
	
	@Test
	public void testSize() {
		try {
			int test = artist.size();
			Album a1 = new Album(artist, ALBUM_NAME);
			artist.addAlbum(a1);
			int test2 = artist.size();
		} catch (Exception e) {
			Assert.fail("Artist.size() did not return an int.\n");
		}
	}
	
	@Test
	public void testEquals() {
		String notAnArtist = "I'm not an artist";
		Assert.assertFalse(artist.equals(notAnArtist) );
